...
//             <div>
//               <h3 className="font-semibold text-indigo-600 mb-3">Checklist</h3>
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div ref={contentRef}>
          ...
          <Card className="bg-white/80 backdrop-blur-md shadow-xl rounded-2xl">
            ...
            <CardContent className="grid grid-cols-2 gap-6 text-sm text-gray-700">
              <div>
                <strong>Registration:</strong> {inspection.vehicle?.registration_number}
              </div>
              <div>
                <strong>Make/Model:</strong> {inspection.vehicle?.make} {inspection.vehicle?.model}
              </div>
              <div>
                <strong>VIN:</strong> {inspection.vehicle?.vin_number}
              </div>
          ...
          <Card className="bg-white/80 backdrop-blur-md shadow-xl rounded-2xl">
            ...
            <CardContent className="grid grid-cols-2 gap-6 text-sm text-gray-700">
              <div>
                <strong>Name:</strong> {inspection.driver ? `${inspection.driver.first_name} ${inspection.driver.surname}` : "N/A"}
              </div>
              <div>
                <strong>License No:</strong> {inspection.driver?.license_number}
              </div>
              <div>
                <strong>License Expiry:</strong> {inspection.driver?.license_expiry_date}
              </div>
              <div>
                <strong>Cell:</strong> {inspection.driver?.cell_number}
              </div>
          ...
          <Card className="bg-white/90 backdrop-blur-md shadow-xl rounded-2xl">
            ...
            <CardContent className="space-y-6 text-sm text-gray-700">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  <strong>Date:</strong> {new Date(inspection.inspection_date).toLocaleString()}
                </div>
              </div>
              ...
              <div>
                ...
                {Array.isArray(inspection.checklist) && inspection.checklist.length > 0 ? (
                      ...
                      <ul className="list-disc list-inside space-y-2">
                        {section.items.map((item: any, iIdx: number) => (
                          <li key={iIdx} className="flex justify-between">
                            <span>
                              {item.label}{" "}
                              <span className="text-gray-500">(Cat {item.category})</span>
                            </span>
                            <span
                              className={
                                item.status === "Faulty"
                  ...
                  <p className="text-gray-500 italic">No checklist data available.</p>
                )}
              </div>
            </CardContent>
          </Card>
